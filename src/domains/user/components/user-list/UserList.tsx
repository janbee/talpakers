...
const UserListComponent: FC = () => {
  const { list, loading, handleOrderByStatus, statusCount, restrictedCount } =
    useUserList();
  const navigate = useNavigate();
  const location = useLocation();

  const selectedUserMemo = useMemo<Map<string, boolean>>(() => {
    const usersFromUrl = location.pathname
      .replace('/users/', '')
      .split(',')
      .filter(Boolean);
  ...
  return (
    <div
      ...
      className={
        'w-full m-4 bg-neutral-800 rounded-lg relative [&:has(+[data-testid="UserDetails"])]:mr-0'
      }
    >
      <div className={'flex flex-col p-4 h-full'}>
        <div className={'flex flex-row items-start justify-between h-12'}>
          <span className={'dark:text-white text-2xl'}>
            Users {selectedUserMemo.size > 1 && selectedUserMemo.size}
          </span>
          <Icon
            circular
            inverted
            ...
            >
              <TableRow>
                ...
                <TableHeaderCell
                  collapsing
                  textAlign={'center'}
                  className={'min-w-[205px] relative'}
                >
                  Weekly Summary <br />
                  (Bonus + Earnings = Total)
                  <Button
                    className={
                      'absolute !text-[9px] top-0 right-0 bottom-0 left-0 opacity-0'
                    }
                    inverted
                    onClick={handleOrderByStatus}
                    filter={'earnings'}
                ...
                <TableHeaderCell
                  collapsing
                  textAlign={'center'}
                  className={'min-w-[105px] relative'}
                >
                  Next <br />
                  Withdrawal
                  <Button
              ...
              {list.map((user) => {
                return (
                  ...
                  >
                    ...
                    <AppBuildCell
                      className={'md:!min-w-[33%] md:!text-center'}
                      user={user}
                    />
                    <StatusCell
                      className={'md:!min-w-[33%] md:!text-right'}
                      textAlign={'center'}
                      user={user}
                    />
                    ...
                    {!!restrictedCount && (
                      <BetRestrictedCell textAlign={'center'} user={user} />
                    )}
                ...
                <TableHeaderCell colSpan={100}>
                  ...
                  >
                    ...
                    <div className={'filter-wrap'}>
                      ...
                      >
                        Done{' '}
                        {!!statusCount[UserStatusModel.IsDone] &&
                          `#${statusCount[UserStatusModel.IsDone]}`}
                      </Button>
                      <Button
                        compact
                      ...
                      >
                        InProgress{' '}
                        {!!statusCount[UserStatusModel.InProgress] &&
                          `#${statusCount[UserStatusModel.InProgress]}`}
                      </Button>
                      <Button
                        compact
                      ...
                      >
                        Waiting{' '}
                        {!!statusCount[UserStatusModel.IsWaiting] &&
                          `#${statusCount[UserStatusModel.IsWaiting]}`}
                      </Button>
                    </div>
                  </div>
