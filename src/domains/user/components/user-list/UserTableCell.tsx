              ...
              trigger={
                <div
                  className={classNames({
                    'has-withdrawal rounded-full h-2 w-2 top-2 right-2 absolute cursor-pointer':
                      true,
    ...
    <TableCell {...omit(props, ['user'])}>
      {[userStatus].map((s, statusInd) => {
        ...
        let FIcon = (
          <i
            key={statusInd}
            className="fa-solid fa-circle-check  text-green-dark cursor-pointer"
          />
        );
        if (s === UserStatusModel.InProgress) {
          ...
          FIcon = (
            <i
              key={statusInd}
              className="fa-solid fa-basketball  text-yellow-dark cursor-pointer"
            />
          );
          ...
          FIcon = (
            <i
              key={statusInd}
              className="fa-solid fa-circle-stop  text-red-dark cursor-pointer"
            />
          );
        }
        return (
...
export const WeeklyProgressCell: FC<UserTableCellProps> = (props) => {
  ...
  let totalStaked =
    user.data?.weeklyStatus?.highestTotalStaked ??
    user.data.weeklyStatus?.betSummary.totalStaked ??
    0;
  if (isNewWeek) {
    totalStaked = 0;
  }
...
export const NextWithdrawalCell: FC<UserTableCellProps> = (props) => {
  ...
  const fixedAmount =
    (user.data.userSession?.autoCashout?.fixedAmount ?? 900) + maintainCash;
  ...
  const bgColor =
    nextCashOutPercent > 30
      ? GetColorUtil(29)
      : GetColorUtil(Math.floor(nextCashOutPercent));
        ...
        <Popup
          ...
          trigger={
            <span style={{ color: bgColor }}>
              {MoneyUtil(cashout.toFixed(0), { minimumFractionDigits: 0 })}
            </span>
          }
          flowing
        >
          <Popup.Header>
            <span className={'text-green-light'}>
              Current Cash - {MoneyUtil(cash, { minimumFractionDigits: 0 })}
            </span>
          </Popup.Header>
          <Popup.Header>
            <span className={'text-green-light'}>
              Cashout at - {`${cashoutPercent}%`}
            </span>
          </Popup.Header>
        </Popup>
        <span>/</span>
...
export const BetRestrictedCell: FC<UserTableCellProps> = (props) => {
  ...
  const hasBetRestriction = isNewWeek
    ? null
    : !!user.data.weekStatus?.hasBetRestriction;
  ...
  return (
    <TableCell className={'relative'} {...omit(props, ['user'])}>
      {hasBetRestriction && (
        <Popup
          position="left center"
          trigger={<span className={'text-red-dark'}>true</span>}
          flowing
        >
          <Popup.Header>
            <span className={'text-red-light'}>
              {dayjs(user.data.weekStatus?.hasBetRestriction).fromNow()}
            </span>
          </Popup.Header>
        </Popup>
      )}
        ...
        <Popup
          ...
          trigger={
            <span className={'text-green-dark'}>
              {MoneyUtil(bonus.Amount, { minimumFractionDigits: 0 })}
            </span>
          }
          flowing
        >
    ...
    <TableCell className={'relative'} {...omit(props, ['user'])}>
      <Popup
        ...
        trigger={
          <span>
            {!!freeBets.length && (
              <span className={'text-green-dark'}>{freeBets.length}</span>
            )}
          </span>
        }
        flowing
...
export const ActiveCell: FC<UserTableCellProps> = (props) => {
  ...
  const bgColor =
    minutesPassed > 30
      ? GetColorUtil(29)
      : GetColorUtil(Math.floor(minutesPassed));
    ...
    <TableCell className={'relative'} {...omit(props, ['user'])}>
      <Popup
        ...
        trigger={
          <div
            className={classNames({
              'rounded-full h-2 w-2 top-2 right-2 absolute cursor-pointer':
                hasBetRestriction,
              'bg-red-light': true,
            })}
          />
