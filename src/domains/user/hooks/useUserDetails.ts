...
const useUserDetails = () => {
  ...
  useEffect(() => {
    ...
    const userDetails$ = forkJoin([
      SharedApi.getBetSummary({ email: { $in: emailArr } }),
      SharedApi.getBonuses({ email: { $in: emailArr } }),
      SharedApi.getWithdrawals({ email: { $in: emailArr } }),
      SharedApi.getUser({ email: { $in: emailArr } }),
    ]).subscribe({
      next: ([betSummaryList, bonusList, withdrawalList, userDetails]) => {
        ...
        const weeksGroupedByMonKeys = Object.keys(weeksGroupedByMon).map(
          (key) => {
            return {
              title: key.split('-')[1],
...
const getWeeksForCurrentYear = ({
  ...
  return Array.from({ length: isoWeeksNumber }, (_, i) => i).map(
    (weekNumber) => {
      ...
      const betSummary = getBetSummaryByWeek(
        betSummaryList,
        weekStart,
        weekEnd,
        year,
      );
      const { betSummaryByWeek, bonus, totalStaked, totalEarnings } =
        betSummary;
      let { approxWinnings } = betSummary;

      const bonusByWeek = getBonusByWeek(
        userBonusList,
        weekStart,
        weekEnd,
        emailArr,
      ...
      const withdrawalByWeek = getWithdrawalByWeek(
        userWithdrawalList,
        weekStart,
        weekEnd,
      );
...
const getBetSummaryByWeek = (
  ...
  const betSummaryByWeek =
    betSummaryList?.filter((item) => {
      return (
        item.startDate ===
          dayjs(weekStart).utc().startOf('day').toISOString() &&
  ...
  return emailArr
    ?.map((email) => {
      return userBonusList?.reverse().find((item) => {
        const TransactionDateTime = dayjs(item.TransactionDateTime)
          .startOf('day')
          .subtract(7, 'days');
        return (
