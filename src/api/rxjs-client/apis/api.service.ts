...
import {
  BetSummaryModel, BonusModel, MongoCollection, PredictionModel, SettledBetModel, UserDetailModel, WithdrawalModel
...
class ApiService {
  ...
  getUsers ({
    filter,
    sort
  }: {filter?: Record<string, unknown>; sort?: Record<string, number>}) {
    return this.$RealmDB
      .collection(MongoCollection.User)
      .getBy({
        filter,
